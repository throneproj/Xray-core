...
//
// RegisterOutbound only reaches SocketConfigs the wiring knows about, so DialSystem
// folds the same bind into whatever SocketConfig it is handed (withEgressBind),
// ahead of every branch so no early return can skip it. That covers callers
// carrying a config the wiring never saw — the udphop finalmask dials on its own
// mask-level sockopt, Xray's internal DNS transports pass none at all — and any
// Xray adds later. The two layers are complementary, not alternatives: this one
...
func bindStreamEgress(mss *MemoryStreamConfig, iface string, mark uint32) {
	mss.SocketSettings = withEgressBind(mss.SocketSettings, iface, mark)
}

// withEgressBind returns sc carrying the given interface and mark, cloning rather
// than mutating so a concurrent dial reading the old pointer is unaffected. It
// returns sc unchanged when it already carries them, which is the common case:
// outbounds registered with RegisterOutbound were bound once and re-dial often.
func withEgressBind(sc *SocketConfig, iface string, mark uint32) *SocketConfig {
	if sc != nil && sc.Interface == iface && (mark == 0 || sc.Mark == int32(mark)) {
		return sc
	}
	var out *SocketConfig
	if sc != nil {
		out = proto.Clone(sc).(*SocketConfig)
	} else {
		out = &SocketConfig{}
	}
	out.Interface = iface
